...
When Claude Code can't return a resumed session to its worktree, it says so with one of the messages below.
...
| `Did not re-enter your worktree <path>`           | Claude Code refused the worktree binding as unsafe; it clears the binding and the session continues in the current directory without isolation. The message includes the specific refusal: match it under [Claude Code refuses to use a worktree](#claude-code-refuses-to-use-a-worktree), since the fix is recreation for some refusals and a path change for others.                                                                              |
...
In [non-interactive mode](/docs/en/headless) with `-p`, Claude Code stops the resume with an error for every refusal except a gone worktree, instead of continuing without isolation. For a worktree that no longer exists, Claude Code prints a notice and continues the session, the same as an interactive resume.
